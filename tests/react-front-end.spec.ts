...
/**
 * React Front-End – 5 Scenario Test Cases + 1-Hour Soak Runner
 *
 * Scenario 1: Login Functionality
 *   TC-RE-001 – Login to eColane React Platform, verify credentials, session active
 *
 * Scenario 2: Create Trips for Client 28325 (future trips)
 *   TC-RE-002 – Navigate to Clients → 28325 → New Trip, fill fields, create trip
 *
 * Scenario 3: Create a New Client
 *   TC-RE-003 – Navigate to Administration → Clients → New Client, fill all mandatory fields, create
 *
 * Scenario 4: Batch Optimization
 *   TC-RE-004 – Navigate to Operations → Schedules → Optimize Schedules, start batch optimization
 *
 * Scenario 5: Send Messages
 *   TC-RE-005 – Navigate to Operations → Messages → Send Message, fill details, send
 *
 * Soak Test:
 *   TC-RE-SOAK – Execute all 5 scenarios in a loop for 1 hour, then stop and report
 */
 ...
 * Returns a string like "Friday, June 20," (note trailing comma — matches
...
function createPerfTracker(scenarioId: string) {
  ...
  return {
    ...
    report(): void {
      ...
      const dbl  = '═'.repeat(68);
      console.log(`\n${dbl}`);
      console.log(`  ⏱  PERFORMANCE REPORT — ${scenarioId}`);
      console.log(dbl);
...
async function loginToApp(page: Page, tag: string): Promise<void> {
  ...
  if (onLoginPage) {
    ...
    console.log(`[${tag}] Session already active — redirected to: ${currentUrl}`);
...
/**
 * Scenario 1 – Login to eColane React Platform and verify session is active.
 * Steps:
...
async function runScenario1(page: Page, perfLabel = 'TC-RE-001'): Promise<void> {
  ...
  await page.waitForLoadState('domcontentloaded');
  await page.waitForLoadState('networkidle', { timeout: 15_000 }).catch(() => {});
  perf.step('Step 1 – Navigate to React DRT URL');
  ...
  const onLoginPage = currentUrl.includes('/login') ||
    await page.getByRole('textbox', { name: 'Username' }).isVisible({ timeout: 3_000 }).catch(() => false);
  perf.step('Step 2 – Detect login/session state');
  ...
  if (onLoginPage) {
    ...
    const signInBtn = page.getByRole('button', { name: 'Log in' });
    await expect(signInBtn).toBeVisible({ timeout: 10_000 });
    perf.step('Step 3 – Verify login form elements');
    ...
    await page.waitForLoadState('domcontentloaded');
    perf.step('Step 4 – Log in & redirect to main app');
  } else {
    // Already authenticated — session is active
    console.log(`[${perfLabel}] Session already active — redirected to: ${currentUrl}`);
    expect(currentUrl).not.toMatch(/\/login/i);
    perf.step('Step 3-4 – Session already active (no login required)');
  }

  // Step 5: Verify session is active — main navigation is accessible
  const mainNav = page.locator('nav, [role="navigation"]').first();
  await expect(mainNav).toBeVisible({ timeout: 15_000 });
  expect(page.url()).not.toMatch(/\/login/i);
  perf.step('Step 5 – Verify session active & nav visible');

  perf.report();
}

/**
 * Scenario 2 – Create trip for Client 28325 with mandatory fields.
 * Steps:
 *   1. Login
...
async function runScenario2(page: Page, perfLabel = 'TC-RE-002'): Promise<void> {
  ...
  perf.step('Step 1 – Login');

  // Step 2: Navigate to Clients list
  await page.getByRole('link', { name: 'Clients' }).click();
  await page.waitForLoadState('domcontentloaded');
  await page.waitForLoadState('networkidle', { timeout: 15_000 }).catch(() => {});
  perf.step('Step 2 – Navigate to Clients page');
  ...
  perf.step('Step 3 – Search for client 28325');
  ...
  await page.waitForLoadState('domcontentloaded');
  await page.waitForLoadState('networkidle', { timeout: 20_000 }).catch(() => {});
  perf.step('Step 4 – Open New Trip form');
  ...
  perf.step('Step 5 – Select future date from calendar');

  // Step 6: Fill mandatory trip fields — time, pick-up address, drop-off address
  ...
  perf.step('Step 6 – Fill mandatory trip fields');
  ...
  perf.step('Step 7-8 – Create trip & verify success');

  perf.report();
}

/**
 * Scenario 3 – Create a new client with all mandatory fields.
 * Steps:
 *   1. Login
...
async function runScenario3(page: Page, perfLabel = 'TC-RE-003'): Promise<void> {
  ...
  // Step 2: Navigate to Administration → Clients
  ...
  perf.step('Step 3 – Open New Client form');
  ...
  perf.step('Step 4 – Fill all mandatory fields');
  ...
  perf.step('Step 5 – Submit Create & verify success');

  perf.report();
}

/**
 * Scenario 4 – Batch Optimization – Optimize schedules.
 * Steps:
 *   1. Login
 *   2. Navigate to Operations → Schedules
...
async function runScenario4(page: Page, perfLabel = 'TC-RE-004'): Promise<void> {
  ...
  perf.step('Step 2 – Navigate to Schedules page');
  ...
  await page.waitForLoadState('domcontentloaded');
  await page.waitForLoadState('networkidle', { timeout: 10_000 }).catch(() => {});
  perf.step('Step 3 – Click Optimize schedules button');
  ...
  await page.waitForLoadState('domcontentloaded');
  await page.waitForLoadState('networkidle', { timeout: 20_000 }).catch(() => {});
  perf.step('Step 4 – Start batch optimization');
  ...
  perf.step('Step 5 – Verify optimization result');

  perf.report();
}

/**
 * Scenario 5 – Send Messages to a vehicle and a provider.
 * Steps:
 *   1. Login
 *   2. Navigate to Operations → Messages
...
async function runScenario5(page: Page, perfLabel = 'TC-RE-005'): Promise<void> {
  ...
  await page.waitForLoadState('domcontentloaded');
  await page.waitForLoadState('networkidle', { timeout: 15_000 }).catch(() => {});
  perf.step('Step 2 – Navigate to Messages page');

  // Step 3: Click "Send message" button to open the modal
  await page.getByRole('button', { name: 'Send message' }).click();
  await page.waitForLoadState('networkidle', { timeout: 10_000 }).catch(() => {});
  perf.step('Step 3 – Open Send Message modal');
  ...
  try {
    ...
    console.log(`[${perfLabel}] Vehicle option '01ahvan' not found — skipping scenario`);
    perf.step('Step 4 – Skipped (vehicle option not found)');
    perf.report();
    return;
  }
  perf.step('Step 4 – Select vehicle recipient (01ahvan)');
  ...
  perf.step('Step 5 – Select provider recipient (AutoProvider_1666)');
  ...
  perf.step('Step 6 – Select message template');
  ...
  perf.step('Step 7 – Click Send message button');

  /*
  // Step 8: Verify success — modal closes or success indicator appears
  await page.waitForLoadState('networkidle', { timeout: 15_000 }).catch(() => {});
  ...
  console.log(`[${perfLabel}] Message sent — modal dismissed: ${modalGone}, success toast: ${hasSuccessToast}`);
  expect(modalGone || hasSuccessToast).toBe(true);

  await expect(
    page.getByText(/unexpected error|server error/i).first(),
  ).not.toBeVisible({ timeout: 3_000 });
  perf.step('Step 8 – Verify message sent successfully');*/
...
test('TC-RE-001: Scenario 1 – Login to eColane React Platform and verify session is active',
  { tag: ['@login', '@smoke', '@react'] },
  async ({ page }) => {
    await runScenario1(page);
  });

test('TC-RE-002: Scenario 2 – Create trip for Client 28325 with mandatory fields',
  { tag: ['@trips', '@clients', '@crud', '@smoke', '@react'] },
  async ({ page }) => {
    await runScenario2(page);
  });

test('TC-RE-003: Scenario 3 – Create a new client with all mandatory fields',
  { tag: ['@clients', '@administration', '@crud', '@smoke', '@react'] },
  async ({ page }) => {
    await runScenario3(page);
  });

test('TC-RE-004: Scenario 4 – Batch Optimization – Optimize schedules',
  { tag: ['@optimization', '@operations', '@smoke', '@react'] },
  async ({ page }) => {
    await runScenario4(page);
  });

test('TC-RE-005: Scenario 5 – Send Messages to vehicle and provider',
  { tag: ['@messages', '@operations', '@smoke', '@react'] },
  async ({ page }) => {
    await runScenario5(page);
  });

// ─────────────────────────────────────────────────────────────────────────────
// Soak Test – Execute all 5 scenarios in a loop for 1 hour, then stop
//
// Runs Scenario 1 → 2 → 3 → 4 → 5 sequentially, repeating the cycle until
...
test('TC-RE-SOAK: 1-Hour Endurance Run – All 5 Scenarios',
  { tag: ['@soak', '@endurance', '@react'] },
  async ({ page }) => {
    ...
    console.log('\n' + '█'.repeat(68));
    console.log('  🚀  SOAK TEST STARTED — TC-RE-SOAK');
    ...
    while (performance.now() - soakStart < SOAK_DURATION_MS) {
      ...
      stats['Scenario 1 – Login'].push(performance.now() - t0);
      ...
      stats['Scenario 2 – Create Trip'].push(performance.now() - t0);
      ...
      stats['Scenario 3 – New Client'].push(performance.now() - t0);
      ...
      stats['Scenario 4 – Batch Optim.'].push(performance.now() - t0);
      ...
      stats['Scenario 5 – Send Message'].push(performance.now() - t0);

      const iterMs = performance.now() - iterStart;
      console.log(`── Iteration ${iteration} complete in ${(iterMs / 1_000).toFixed(1)}s ──────────────────────`);
    }
    ...
    console.log(`\n${dbl}`);
    console.log(`  🏁  SOAK TEST COMPLETE — TC-RE-SOAK`);
    ...
    for (const [name, durations] of Object.entries(stats)) {
      if (durations.length === 0) {
        console.log(`  ${name.padEnd(28)}${'0'.padStart(6)}${'—'.padStart(10)}${'—'.padStart(10)}${'—'.padStart(10)}${'—'.padStart(11)}`);
        continue;
      }
